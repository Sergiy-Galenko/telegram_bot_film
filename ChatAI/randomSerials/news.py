...
class RandomNewsSeries:
    ...
    def get_random_news_series(self):
        data = requests.get(f"https://api.themoviedb.org/3/discover/tv?api_key={self.api_key}&with_genres=10763").json()
        if data['results']:
...
if (series := RandomNewsSeries(api_key).get_random_news_series()): [print(f"{k.capitalize()}: {v}") for k, v in series.items()]
