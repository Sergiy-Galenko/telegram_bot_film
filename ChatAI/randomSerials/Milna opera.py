...
class RandomSoapSeries:
    def __init__(self, api_key):
        self.api_key = api_key
    def get_random_soap_series(self):
        data = requests.get(f"https://api.themoviedb.org/3/discover/tv?api_key={self.api_key}&with_genres=10766").json()
        if data['results']:
...
if (series := RandomSoapSeries(api_key).get_random_soap_series()): [print(f"{k.capitalize()}: {v}") for k, v in series.items()]
